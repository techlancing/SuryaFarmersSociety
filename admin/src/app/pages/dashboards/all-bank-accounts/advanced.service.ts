 ...
 */
function sort(tables: BankAccount[], column: string, direction: string): BankAccount[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: BankAccount, term: string, pipe: PipeTransform) {
    return tables.sAccountNo.toLowerCase().includes(term.toLowerCase());
        //|| tables.nAnswer.toLowerCase().includes(term)
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<BankAccount[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    public tableData : Array<BankAccount>;
