...
import { toApiDate, aggregateByBranchAndCategory } from '../api/reportUtils'
...
export function AllCategoryBalanceSummaryPage() {
  const [showReport, setShowReport] = useState(false)
  const [rows, setRows] = useState<Array<{ branch: string; category: string; nCreditAmount: number; nDebitAmount: number; nBalanceAmount: number }>>([])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const aggregated = aggregateByBranchAndCategory(txns)
      setRows(aggregated)
  ...
  return (
    <div className="page">
      <Typography variant="h5" gutterBottom>All Category Balance Summary</Typography>
      <Typography variant="body2" color="text.secondary" gutterBottom>
        Select date range to get balance summary for all branches and categories. Branch is derived from account number prefix.
      </Typography>
