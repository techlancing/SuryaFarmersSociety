...
import { toApiDate, aggregateByCategory } from '../api/reportUtils'
...
export function CategoryBalanceSummaryPage() {
  const [showReport, setShowReport] = useState(false)
  const [rows, setRows] = useState<Array<{ category: string; nCreditAmount: number; nDebitAmount: number; nBalanceAmount: number }>>([])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const aggregated = aggregateByCategory(txns)
      setRows(aggregated)
  ...
  return (
    <div className="page">
      <Typography variant="h5" gutterBottom>Category Balance Summary</Typography>
      <Typography variant="body2" color="text.secondary" gutterBottom>
        Select date range to get balance summary by category. To date must be on or after from date.
      </Typography>
