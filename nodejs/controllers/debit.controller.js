...
oDebitRouter.post("/add_debit", oAuthentication, asyncMiddleware(async (oReq, oRes, oNext) => {
  ...
  //checking the creditloan and account are related
    ...
    const pendingtransaction = await oTransactionModel.find({ nLoanId: newDebit.nLoanId, sIsApproved: 'Pending' }).sort({ _id: -1 }).limit(1);
    if (pendingtransaction.length > 0) {
      ...
      if (oReq.body.nAmount > oBalanceAmount) oRes.json({ status: "low", msg: "outstanding balance is low" });
      else {
